...
| 主页 | 已绑定文档按源路径**目录树**展示 + **绑定缺失**分区及操作入口 |
| 漂移检测 | 改名自动更新路径；哈希变化仅作文档核对提醒（可忽略）；行范围/符号等需处理 |
...
按绑定的源文件路径展开为目录树（文件夹 → 源文件 → 旁路文档）；同一源上的多个绑定（整文件 / 代码块）挂在同一文件节点下。汇总索引单独置顶。
...
| 行范围失效 | range 行号越界 | 重新绑定、打开文档 |
| 符号变动 | symbol 找不到或已移出绑定行范围 | 重新绑定、打开文档 |
