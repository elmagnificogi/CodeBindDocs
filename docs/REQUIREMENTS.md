...
CodeBind Docs（简称 **CBD**）是 VS Code / Cursor 扩展：用**旁路绑定**把设计文档与源码关联起来，在编辑器中**左右分栏**同步查看与编辑，且不修改原始源码。
...
| 主页 | 已绑定目录树 + **绑定覆盖率摘要** + **绑定缺失**分区；未绑定列表在独立页 |
...
| Code | 文档面板「Code」跳转到绑定文件；range 时选中整段行范围 |
| 文档模板 | 新建绑定时可选模板；目录由 **`cbd.templatesPath`**（默认 `{docsPath}/_templates`）配置，支持 `{{title}}` |
| 文档资源 | 粘贴/上传图片自动写入 **`cbd.assetsPath`**（默认 `{docsPath}/assets`），Markdown 用相对路径引用 |
| 文档大纲 | IR 模式可选右侧 TOC（`cbd.docPane.outline`，默认开） |
| 文档嵌入 | `cbd-include` 围栏只读嵌入本仓库其它 Markdown |
| Agent | `AGENTS.md` + `.cursor/rules/cbd.mdc`；模块→文档对照表 |
...
docs/REQUIREMENTS.md  # 产品需求（无 cbd 头，不算绑定）
```
...
可选 `lines: 10-40`。面板加载时展开为只读预览；保存时写回紧凑 `cbd-include`（展开内容不持久化进宿主文档）。

## UX 细节

### 文档面板

- 左：源码；右：CodeBind Docs 面板（主页 / 文档 / 无关联）
- 启动/窗口恢复后补做分栏同步（首个已聚焦页签不会触发 activeEditor 变更）
- **即时渲染**：Vditor IR（复用实例；`setValue` 清栈，避免切换残留）；可选右侧大纲（TOC）
- **文档源码**：普通 textarea，非 Vditor 分屏（避免切换卡顿）
- YAML 文件头不进入编辑区；保存时拼回
- 围栏内 `---` 加载前做保护，保存前还原（`mdProtect`）
- 粘贴/上传图片：写入 `assets/`，正文写入相对路径；面板内用 webview URI 预览
- 导航：主页 / 后退 / 前进
- 可删文档时显示红色「删除」按钮
- 绑定文档显示「Code」（range 时选中整段）

### 主页 · 已绑定文档

按绑定的源文件路径展开为目录树（文件夹 → 源文件 → 旁路文档）；同一源上的多个绑定（整文件 / 代码块）挂在同一文件节点下。**目录绑定挂在对应文件夹下**，不把该文件夹渲染成文件。汇总索引单独置顶。文件夹折叠状态在会话内记忆；点击源文件名打开 Code，点击文档打开旁路文档。

### 主页 · 绑定覆盖率

扫描可绑定源文件（常见语言后缀，排除 `node_modules` / `out` / 文档目录等）。主页仅显示已绑定比例摘要；完整未绑定列表在独立页，可从主页「查看未绑定」进入，并支持「新建绑定」或打开源文件。
...
| 范围重叠 | 同文件多个 range 行范围相交 | 打开文档、Code、重新绑定 |
...
状态栏：绑定异常显示警告；仅核对提醒时显示 `CodeBind Docs 核对 N`（无警告底色）。
...
- 入口：面板「删除」、侧栏右键、命令面板 `CBD: Delete Bound Doc`
- **一次确认**后删除
...
| 命令 | 作用 |
|------|------|
| `CBD: Initialize` | 文档目录与 Agent 脚手架 |
...
- `.cursor/rules/cbd.mdc`：编辑绑定源文件前先读文档
- 文档为普通 Markdown；改行为须同步更新对应旁路文档
- 本仓库 dogfood：见 `docs/cbd-index.md`

## 成功标准

1. `npm run debug` 后，打开已绑定源文件可见左右分栏文档；range 随光标切换。
2. 绑定与文档可提交 Git；源码无 CodeBind Docs 标记污染。
3. 改名可更新 `cbd.target`，或主页/弹窗提示缺失并可改绑/删除。
4. 重新绑定支持整文件与代码块选区。
5. 删除需确认；`cbd-index` 不可删。
6. 源文件保存后，内容/行范围/符号变动有可操作提示。
7. Agent 规则与对照表存在；`src/**` 均有绑定文档。
8. 即时渲染与文档源码切换流畅；新建文档顶部无多余「幽灵代码块」。
