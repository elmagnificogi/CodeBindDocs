...
- 主页绑定树不再把目录绑定渲染成文件节点，避免挡住该目录下的文件 / 代码块绑定
- 覆盖率统计不再把 `kind: directory` 的路径算作已绑定源文件

## [0.1.17] - 2026-08-12

### Fixed

- `CBD: Refresh Doc Tree` 会真正重扫绑定并刷新侧栏；主页/覆盖率页打开时也会就地重绘（不再因缓存旧列表而看似未刷新）

### Changed

- 设置项说明改为中英双语（`cbd.*` 配置界面）
- 移除 `docs/cbd/agent-scaffold.md` 绑定文档（有意删除）
...
- `CBD: Reveal Bound Doc`（`Ctrl+Alt+D`）在无绑定时直接打开「无关联文档」面板以便新建，不再弹出提示框；关闭自动分栏时同样生效

## [0.1.14] - 2026-07-28

### Fixed

- 绑定文档被外部修改（Git、外部编辑器、Agent 等）后，CBD 面板自动刷新；切换源码文件再切回时不再显示旧内容
- autosave 不再用面板内陈旧内容覆盖磁盘上的外部修改

## [0.1.13] - 2026-07-28

### Added

- 当前代码的祖先目录存在目录绑定时，关联文档顶部显示「目录文档」按钮，并打开最近一层目录文档

### Fixed

- `cbd.docPane.outline` 在已打开的关联文档中实时切换大纲，并同步显示或隐藏工具栏入口
- 复用或重新显示已有文档面板时重新同步大纲设置，避免保留面板沿用旧状态

## [0.1.12] - 2026-07-27

### Changed

- README 对齐当前能力：安装入口、默认 `docs/cbd/`、目录绑定、快捷键与多根工作区说明
